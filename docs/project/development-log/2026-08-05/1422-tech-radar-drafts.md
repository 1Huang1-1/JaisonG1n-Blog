...
## 学习资料库追加（同日，大学经典课程/教材）

- 新增 12 条草稿，ID 范围 266–277，状态均为 `draft`。
- 数据结构：严蔚敏《数据结构（C语言版）》+ 浙大陈越《数据结构》MOOC；算法：MIT 6.006 + CLRS《算法导论》；计网：谢希仁《计算机网络》+ Kurose《自顶向下方法》；操作系统：恐龙书 + MIT 6.S081；组成原理：唐朔飞《计算机组成原理》；数据库：CMU 15-445；编译原理：龙书；数学：MIT 18.06 线性代数。
- 官方链接均已做可用性检查（MIT Press 返回 403 为反爬拦截，链接本身有效）。
