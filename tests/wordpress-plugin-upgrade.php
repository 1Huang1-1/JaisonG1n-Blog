...
jg_upgrade_assert($baseline_validation === 0, '0.10.0 baseline validate_plugin failed: ' . wp_json_encode($baseline_validation));
$activation = activate_plugin($basename, '', false, false);
jg_upgrade_assert(!is_wp_error($activation), '0.10.0 activation failed: ' . wp_json_encode($activation));
...
jg_upgrade_assert(get_option('jg_site_settings', array())['site_title'] === 'Upgrade preserved', '0.10.0 setting fixture is missing.');
jg_upgrade_assert(count(get_posts(array('post_type' => 'jg_project', 'post_status' => 'publish', 'numberposts' => -1))) === 1, '0.10.0 content fixture is missing.');
jg_upgrade_assert((int) get_option('jg_upgrade_ai_draft', 0) > 0, '0.10.0 AI draft fixture is missing.');
...
jg_upgrade_assert($replacement_validation === 0, '0.12.0 validate_plugin failed: ' . wp_json_encode($replacement_validation));

require $main_file;
jg_upgrade_assert(defined('JG_SITE_MANAGER_VERSION') && JG_SITE_MANAGER_VERSION === '0.12.0', '0.12.0 plugin did not load.');
...
jg_upgrade_assert(JG_Settings::get()['reviewed_article_publish'] === false, 'Reviewed article publishing must default to off after upgrade.');
jg_upgrade_assert(JG_Settings::get()['auto_publishable_ai_articles'] === false, 'Auto-publishable AI articles must default to off after upgrade.');
jg_upgrade_assert(JG_Settings::get()['update_published_diaries'] === false, 'Published diary updates must default to off after upgrade.');
jg_upgrade_assert(JG_Settings::get()['update_published_articles'] === false, 'Published article updates must default to off after upgrade.');
global $wpdb;
...
jg_upgrade_assert(JG_Dispatch::find_latest_record_for_content('diary', 999999, '') === null, 'Legacy dispatch history without contentRefs was bound to content.');
...
jg_upgrade_assert($ai_role && !$ai_role->has_cap('jg_ai_publish_article_drafts'), 'Upgrade unexpectedly granted reviewed article publish permission.');
jg_upgrade_assert($ai_role && !$ai_role->has_cap('jg_ai_update_published_diaries') && !$ai_role->has_cap('jg_ai_update_published_articles'), 'Upgrade unexpectedly granted published-update permissions.');
jg_upgrade_assert($ai_role && $ai_role->has_cap('jg_ai_upload_media') && $ai_role->has_cap('upload_files'), 'AI media upload capabilities are missing after upgrade.');
jg_upgrade_assert($ai_role && !$ai_role->has_cap('manage_options') && !$ai_role->has_cap('edit_others_posts'), 'AI role was expanded beyond the media scope after upgrade.');
